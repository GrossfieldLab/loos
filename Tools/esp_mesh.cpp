...
string fullHelpMessage(void) {
    string msg =
        "SYNOPSIS \n"
"        Fit charges on a mesh composed of spherical shells to reproduce the \n"
"        electrostatic potential due to solvent around a solute. \n"
" \n"
"DESCRIPTION \n"
"        The goal of this tool is to replace solvent configurations sampled from\n"
"        a trajectory with a mesh of point charges representing the mean field \n"
"        electrostatic contribution of the solvent-solute interaction. A common \n"
"        application is producing a set of point charges to represent solvent \n"
"        approximately in a QM/MM calculation. The idea comes from the iPolQ \n"
"        method for deriving aqueous phase partial charges [1]. \n"
" \n"
"        Given an atom selection representing a solute, this tool will generate\n"
"        a mesh of points defined by a union of spherical shells centered on the\n"
"        coordinates of those atoms in the first frame of the trajectory. \n"
"        A mesh of equidistant points on a spherical shell with user-specified \n"
"        radius and point density is generated using a using a Fibonacci \n"
"        spiral [2]. For each atom, these points are rotated to a random \n"
"        orientation using quaternions [3,4] generated using a random point on \n"
"        the surface of a 4D hypersphere [5] and then translated to the \n"
"        coordinates of the atom. Then, points closer than the specified radius \n"
"        to another atom are removed. The result is a mesh of points surrounding\n"
"        and equidistant to the solute atoms. \n"
" \n"
"        Two such meshes are generated at different radii. This tool will \n"
"        compute the classical electrostatic potential (ESP) due to solvent  \n"
"        molecules at each atom in the selection and at each point in the closer\n"
"        mesh. Solvent molecules contribute to the ESP only if at least one atom\n"
"        is closer than a cutoff distance (-C option) but no atom is closer than\n"
"        the farther radius to any solute atom. Then, point charges are placed \n"
"        at points on the farther mesh, and their magnitude is fit by linear \n"
"        regression to reproduce the solvent ESP, averaged over the trajectory, \n"
"        at the points on the closer mesh. The sum of square errors between the \n"
"        ESP at the atoms in the selection due to these point charges and that \n"
"        due to the solvent informs on the quality of the fit. \n"
" \n"
"        Multiple meshes at different radii can be used for the fit point \n"
"        charges. The farthest mesh will be fit first. Then, subsequent meshes \n"
"        will fit the difference between the trajectory-averaged solvent ESP and\n"
"        the ESP due to the fit point charges on farther meshes. The radii are \n"
"        specified in the -R option by passing a string of numbers in descending\n"
"        order separated by spaces. \n"
" \n"
"        The first line of the output is the command-line input. The second line\n"
"        contains the number of points on the closer ESP mesh, the number of \n"
"        points on the farther charge mesh, the sum of the fit point charges, \n"
"        and the sum of square errors from the charge fit. The third line \n"
"        contains the ESP at the solute atoms due to solvent averaged over the \n"
"        trajectoy. The fourth line contains the ESP at the solute atoms due to \n"
"        the fit point charges. Then, for each point on the closer mesh, there \n"
"        will be a line containing the coordinates, the ESP due to solvent \n"
"        averaged over the trajectory, and the ESP due to the fit point charges.\n"
"        Finally, the output contains the coordinates and magnitude of each fit \n"
"        point charge. Units for charges are fundamental charge (e), and units \n"
"        for ESP are kcal mol^-1 e^-1. \n"
" \n"
"        The default selection for the solvent is any residue named \"WAT\" or \n"
"        ending in \"+\" or \"-\". This will catch things like \"Cl-\" or \n"
"        \"Ca2+\" but not \"HOH\" or \"Mg\". The default selection for the \n"
"        solute is any residue not named \"WAT\" or ending in \"+\" or \"-\". \n"
"        However, the solute and solvent selections do not need to be exactly \n"
"        complementary. \n"
" \n"
"        If a solvent other than water is used, the maximum number of atoms per \n"
"        solvent molecule needs to be set using the --solvent_size option. This \n"
"        number needs to be known to allocate memory for a matrix of distances \n"
"        between solvent atoms and mesh points. \n"
" \n"
"        This tool makes no attempt to assess the convergence of the trajectory.\n"
"        Users should check that the solvent ESP is converged with respect to \n"
"        the trajectory length, the sampling interval, and the size of the \n"
"        solvent box. \n"
" \n"
"EXAMPLES \n"
"        esp_mesh foo.pdb foo.dcd \n"
"        Fit a charge mesh at 10 angstroms to reproduce the solvent ESP at \n"
"        3 angstroms for a solute solvated in water and salt. \n"
" \n"
"        esp_mesh -R 4 -S 2.5 foo.pdb foo.dcd \n"
"        Fit a charge mesh at 4 angstroms to reproduce the solvent ESP at \n"
"        2.5 angstroms for a solute solvated in water and salt. \n"
" \n"
"        esp_mesh -R '10 8 6' foo.pdb foo.dcd \n"
"        Fit three charge meshes at 10, 8, and 6 angstroms to reproduce the \n"
"        solvent ESP at 3 angstroms. \n"
" \n"
"        esp_mesh -C 40 foo.pdb foo.dcd \n"
"        Fit a charge mesh at 10 angstroms to reproduce the solvent ESP at \n"
"        3 angstroms. Only solvent molecules between 10 and 40 angstroms will \n"
"        contribute to the solvent ESP. \n"
" \n"
"        esp_mesh -f foo_esp.dat foo.pdb foo.dcd \n"
"        Dump a time series of the solvent ESP at the closer mesh to foo_esp.dat\n"
" \n"
"        esp_mesh --solvent_size 6 -a \"!(resname == 'WAT' || resname == 'MOH')\" \\ \n"
"            -w \"resname == 'WAT' || resname == 'MOH'\" foo.pdb foo.dcd \n"
"        Fit a charge mesh with default parameters for a solute solvatd in a \n"
"        mixture of water and methanol. \n"
" \n"
"        esp_mesh -c foo.pdb foo.dcd \n"
"        Fit a charge mesh allowing solvent molecules in between the closer ESP \n"
"        mesh and the farther charge mesh to contribute to the solvent ESP. Our \n"
"        experience is that this strategy provides a poor approximation for \n"
"        these solvent molecules, so caveat emptor. \n"
" \n"
"NOTES \n"
"        Distances are calculated without periodicity, so this tool assumes that \n"
"        the solute has been centered already. In practice, the solvent box \n"
"        needs to be big enough to include bulk solvent in order for this method \n"
"        to work well, so a box that needs to account for periodic distances is \n"
"        probably too small. \n"
" \n"
"REFERENCES \n"
"        [1] Cerutti DS et al. (2013) J Phys Chem B 117, 2328 \n"
"        [2] Swinbank R & Purser RJ (2006) Q J R Meteorol Soc 132, 1769 \n"
"        [3] Vesely FJ (1982) J Comp Phys 47, 291 \n"
"        [4] Frenkel D & Smit B (2002) Understanding Molecular Simulation, 2e, 49\n"
"        [5] Marsaglia G (1972) Ann Math Stat 43, 645 \n";
    return msg;
}
