...
float upper_expected = num_pdbfiles * num_upper / area;
float lower_expected = num_pdbfiles * num_lower / area;

// Output the results
cout << "# Dist\tTotal\tUpper\tLower" << endl;
for (int i = 0; i < num_bins; i++)
    {
    float d = bin_width*(i + 0.5);

    float d_inner = bin_width*i;
    float d_outer = d_inner + bin_width;
    float norm = M_PI*(d_outer*d_outer - d_inner*d_inner);

    float upper = hist_upper[i]/(norm*upper_expected);
    float lower = hist_lower[i]/(norm*lower_expected);
    // The 2.0 is because there are 2 leaflets, so twice as much area
    float total = (hist_upper[i] + hist_lower[i])/
