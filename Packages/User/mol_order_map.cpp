 ...
 *returned data will be the centroid of the molecules or residues.
 */
...
ValueStore<double> order_upper(num_x_bins, num_y_bins);
ValueStore<double> order_lower(num_x_bins, num_y_bins);
...
while (traj->readFrame())
    ...
    for (int i=0; i<targets.size(); i++)
        ...
        vector<GCoord> target_axes = targets[i].principalAxes();

        double order1 = 0.5 - 1.5 * target_axes[1].z() * target_axes[1].z();
        double order2 = 0.5 - 1.5 * target_axes[2].z() * target_axes[2].z();
        double avg_order = (order1 + order2) / 2;

        if (target_vector.z() > 0)
            {
            counts_upper(x_bin, y_bin)++;
            order_upper(x_bin, y_bin) = order_upper(x_bin, y_bin) + avg_order;
            total_counts++;   
            } else {
            counts_lower(x_bin, y_bin)++;
            order_lower(x_bin, y_bin) = order_lower(x_bin, y_bin) + avg_order;
            total_counts++;
            }
        }
    }
...
for (int k=0; k < num_x_bins; k++)
    ...
    for (int m=0; m < num_y_bins; m++)
        ...
        double order_upper_normalized = 0;
        double order_lower_normalized = 0;
        
        if (counts_upper(k,m) > 0)
            {
            order_upper_normalized = (((double)order_upper(k,m)) / (double)counts_upper(k,m));
            }

        if (counts_lower(k,m) > 0)
            {
            order_lower_normalized = abs(((double)order_lower(k,m)) / (double)counts_lower(k,m));
            }
        ...
        cout << k       << "\t"
             ...
             << m       << "\t"
             << y   << "\t"
             << order_upper_normalized << "\t"
             << order_lower_normalized << "\t"
             << endl;
        }
    cout << endl;
    }
