/*
  Compute the average radius of gyration as a function of time.
...
// JOSH counter for graph titles, cout of other titles
//
//
cout << "#Time\tAvg\tSterr\t";
...
for (m=molecules.begin(); m!=molecules.end(); m++)
    ...
    if (tmp.size() > 0)
        ...
        // JOSH Retrieve segid for output
        //
        //
...
// JOSH vector of values, print headers for graph
//
//
...
while (traj->readFrame())
    ...
    for (m=molecule_groups.begin(); m!=molecule_groups.end(); m++)
        ...
        // JOSH add values to vector
        //
        // 
        series.push_back(m->radiusOfGyration());
    ...
    // JOSH make a time series of this row, compute avg
    // and sd, then print these and the values for the row...
    // repeat for each molecule
    cout << frame << "\t";
    tseries = TimeSeries<double>(series);
    cout << tseries.average() << "\t";
    cout << tseries.sterr() << "\t";
