/*
 *Compute a density map of a selection relative to another set of selections.
 *For instance, the density of cholesterol as a function of position relative
...
double average = (((double)total_counts / 2) / ((max_x - min_x) * (max_y - min_y))) / (double)num_frames;
...
for (int k=0; k < num_x_bins; k++)
    ...
    for (int m=0; m < num_y_bins; m++)
        ...
        double upper_normalized = ((((double)counts_upper(k,m) / (double)num_frames)/ (double)bin_area)) / average;
        double lower_normalized = ((((double)counts_lower(k,m) / (double)num_frames)/ (double)bin_area)) / average;
