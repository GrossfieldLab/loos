/*
  ...
  Aligns along the x-axis. Was useful for lining up fengycin clusters so that
  they are oriented roughly the same way. (Then I could make 2D maps relative
  to fengycin clusters.)
  ...
  ToolOptions() : alignment_string("name == 'CA'"),
    ...
    void addGeneric(po::options_description& o) {
        o.add_options()
            ...
            ("firstRes", po::value<string>(&firstRes)->default_value(lastRes), "First residue of the cluster")
            ("lastRes", po::value<string>(&lastRes)->default_value(firstRes), "Last residue of the cluster");
    }

  string print() const {
    ostringstream oss;
    oss << boost::format("align='%s',transform='%s',maxiter=%d,tolerance=%f,reference='%s',refsel='%s',firstRes='%s',lastRes='%s'")
      % alignment_string % transform_string
      % maxiter % alignment_tol
      % reference_name % reference_sel % firstRes % lastRes;
    return(oss.str());
  }
...
int main(int argc, char *argv[]) {
  ...
  AtomicGroup firstResidue = selectAtoms(model, topts->firstRes);
  AtomicGroup lastResidue = selectAtoms(model, topts->lastRes);
  ...
  if (topts->reference_name.empty()) {
    ...
    for (unsigned int i = 0; i<nframes; i++) {
      ...
      GCoord firstResidueXY = firstResidue.centroid();
      firstResidueXY.z(0);

      GCoord lastResidueXY = lastResidue.centroid();
      lastResidueXY.z(0);

      GCoord vCluster = lastResidueXY - firstResidueXY;

      double theta = acos((vCluster.dot(xAxis)) / (vCluster.length() * xAxis.length())) * (180 / PI);
      ...
      //cout << firstResidueXY << "\t" << lastResidueXY << "\t" << theta << "\t" << vCluster << "\t" << vCluster.dot(xAxis) << endl;

      model.rotate(GCoord(0,0,1), theta);
