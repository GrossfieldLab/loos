...
namespace Clustering
{
  ...
  void vectorVectorsAsJSONArr(std::vector<std::vector<uint>> &clusters, std::ostream &out, std::string indent="  ", std::string offset="  ");
  
  // write single iterable (eg exemplars) as JSON for easy transport to analysis context.
  template <typename ForLoopable>
  void containerAsJSONArr(ForLoopable &container, std::ostream &out, std::string indent="  ", std::string offset="  ");
