...
MatrixXd readMatrixFromStream(istream &input, char commentChar = '#')
{
  ...
  Eigen::Matrix<double, Eigen::Dynamic, Eigen:Dynamic, Eigen::RowMajor> result(matbuff[0].size(), matbuff.size());
...
void vectorVectorAsJSONArr(vector<vector<uint>> &cluster, ostream &out, string indent="  ", string offset="  ")
{ 
  out << offset + "[" << endl;
...
// write iterable container to JSON. Container must have a size() method and a [] operator.
template <typename ForLoopable>
void containerAsJSONArr(ForLoopable &container, ostream &out, string offset="  ", string indent="  ")
{
  out << "[" << endl;
