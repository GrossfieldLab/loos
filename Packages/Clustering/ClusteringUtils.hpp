...
void vectorVectorsAsJSONArr(std::vector<std::vector<uint>> &clusters, std::ostream &out, const std::string &indent="  ", const std::string &offset="  "){};

// write single iterable (eg exemplars) as JSON for easy transport to analysis context.
template <typename ForLoopable>
void containerAsJSONArr(ForLoopable &container, std::ostream &out, const std::string &indent="  ", const std::string &offset="  "){};
